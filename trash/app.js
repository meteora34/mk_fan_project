...
function renderHero() {
  ...
  if (scorp && misc.scorpionHero) scorp.src = misc.scorpionHero;
...
function renderTimeline() {
  ...
  // Re-init intersection observer for newly created items
...
function renderFacts() {
  ...
  if (!grid || !misc.facts) return;

  grid.innerHTML = misc.facts.map(f => `
    <div class="fact-card">
...
function triggerKahnTakeover() {
  ...
  const kahn = fighters.find(f => f.name === 'Shao Kahn');
  if (kahn) sprite.src = kahn.img;
  sprite.style.filter = 'drop-shadow(0 0 60px #8800cc)';
  nameEl.textContent  = 'SHAO KAHN';
  nameEl.style.color  = '#8800cc';
  msgEl.textContent   = 'IT HAS BEGUN';
  overlay.style.background = 'rgba(10,0,20,0.96)';
